...
insulin_data_df_1 = pd.read_csv('Insulin_patient2.csv', low_memory=False,
...
insulin_data_df_1['Date'] = insulin_data_df_1['Date'].str.split().str[0]
...
def extract_meal_data(insulin_data_df, cgm_data_df, dateidentifier):
    insulin_df = insulin_data_df.copy()
    insulin_df = insulin_df.set_index('date_time_stamp')
    sorted_insulin_data = insulin_df.sort_values(by='date_time_stamp',
                                                              ascending=True).dropna().reset_index()
    #print(sorted_insulin_data)
    sorted_insulin_data['BWZ Carb Input (grams)'].replace(0.0, np.nan, inplace=True)
    #print(sorted_insulin_data)
    sorted_insulin_data = sorted_insulin_data.dropna()
    sorted_insulin_data = sorted_insulin_data.reset_index().drop(columns='index')
    #print(sorted_insulin_data)
    valid_timestamp_list = []
    value = 0
    for idx, i in enumerate(sorted_insulin_data['date_time_stamp']):
        try:
            value = (sorted_insulin_data['date_time_stamp'][idx + 1] - i).seconds / 60.0
            if value >= 120:
    ...
    if dateidentifier == 1:
        for idx, i in enumerate(valid_timestamp_list):
            ...
            list1.append(cgm_data_df.loc[cgm_data_df['Date'] == get_date].set_index('date_time_stamp').between_time(
...
meal_data = extract_meal_data(insulin_data_df, cgm_data_df, 1)
meal_data1 = extract_meal_data(insulin_data_df_1, cgm_data_df_1, 2)
meal_data = meal_data.iloc[:, 0:24]
meal_data1 = meal_data1.iloc[:, 0:24]


def extract_no_meal_data(insulin_data_df, cgm_data_df):
    ...
    for idx, i in enumerate(valid_timestamp):
        iteration_dataset = 1
        try:
            length_of_24_dataset = len(cgm_data_df.loc[(cgm_data_df['date_time_stamp'] >= valid_timestamp[
                idx] + pd.Timedelta(hours=2)) & (cgm_data_df['date_time_stamp'] < valid_timestamp[idx + 1])]) // 24
            while (iteration_dataset <= length_of_24_dataset):
                if iteration_dataset == 1:
                    dataset.append(cgm_data_df.loc[(cgm_data_df['date_time_stamp'] >= valid_timestamp[
                        idx] + pd.Timedelta(hours=2)) & (cgm_data_df['date_time_stamp'] < valid_timestamp[idx + 1])][
                                       'Sensor Glucose (mg/dL)'][:iteration_dataset * 24].values.tolist())
                    iteration_dataset += 1
                else:
                    dataset.append(cgm_data_df.loc[(cgm_data_df['date_time_stamp'] >= valid_timestamp[
                        idx] + pd.Timedelta(hours=2)) & (cgm_data_df['date_time_stamp'] < valid_timestamp[idx + 1])][
...
no_meal_data = extract_no_meal_data(insulin_data_df, cgm_data_df)
no_meal_data1 = extract_no_meal_data(insulin_data_df_1, cgm_data_df_1)


def extract_meal_featurematrix(meal_data):
...
meal_feature_matrix = extract_meal_featurematrix(meal_data)
meal_feature_matrix1 = extract_meal_featurematrix(meal_data1)
...
non_meal_feature_matrix = extract_no_meal_featurematrix(no_meal_data)
non_meal_feature_matrix1 = extract_no_meal_featurematrix(no_meal_data1)
...
for train_index, test_index in kfold.split(principaldata):
    X_train, X_test, y_train, y_test = principaldata.loc[train_index], principaldata.loc[test_index], dataset.label.loc[
        train_index], dataset.label.loc[test_index]
